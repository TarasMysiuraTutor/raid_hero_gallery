...
url_main = "https://raid-sl.ru/heroes-cat/heroes"
...
def get_hero_links_from_page(url):
    ...
    for hero in heroes_info_all:
        hero_data = {}
        hero_url = hero.find('a', class_='woocommerce-LoopProduct-link').get('href')
        print(hero_url)
        # image_element = hero.find('img')
        # if image_element:
        #     hero_data["hero_name"] = clean_text(image_element.get("title", "Title not found"))
        #     hero_data["image_url"] = clean_text(image_element.get("src", "URL not found"))
        # link_element = hero.find('a', class_='woocommerce-LoopProduct-link')
        # if link_element:
        #     hero_data["hero_url"] = clean_text(link_element.get("href", "URL not found"))
            # print(hero_data)
    
    # print(heroes_info_all)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        count_heroes=4
        futures = [executor.submit(scrape_to_links_all_pages , heroes_info_all[hero_index]) for hero_index in range(count_heroes)]
...
def scrape_to_links_all_pages(url_main):
    """Скрапінг усіх сторінок з пагінацією."""
    page = 1
    all_data = []
    while True:
        page_url = f"{url_main}/page/{page}"
        print(f"Page URL: {page_url}")
        
        # https://raid-sl.ru/heroes-cat/heroes/page/2
